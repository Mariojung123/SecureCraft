...
function useReportPolling(session_id) {
    ...
    const poll = () => {
      getSessionReport(session_id)
        .then(data => {
          ...
          else { setReport(data); setLoading(false) }
        })
        .catch(() => {
          if (!cancelled) { setError('Report not found. It may have expired.'); setLoading(false) }
        })
    }
    poll()
...
function ReportFailed({ report }) {
  ...
  const lang = report.language ?? 'python'
    ...
    <>
      {report.submitted_code && (
        <section className="mb-8">
          <SectionHeader number="1" color="red" title="Vulnerable Code">
            ...
            {hasVulnLines && (
              <span className="text-xs text-red-400/80 bg-red-900/30 border border-red-800/40 rounded px-2 py-0.5">
          ...
          <HighlightedCode code={report.submitted_code} vulnerableLines={report.vulnerable_lines ?? []} />
          {report.vulnerability_explanation && (
            <div className="mt-3 bg-red-950/30 border border-red-800/40 rounded-xl p-4 text-red-200/80 text-sm leading-relaxed">
              <span className="font-semibold text-red-300">What makes this vulnerable: </span>
              {report.vulnerability_explanation}
            </div>
          )}
          {report.fix_hint && (
            <div className="mt-2 bg-blue-950/30 border border-blue-800/40 rounded-xl p-4 text-blue-200/80 text-sm leading-relaxed">
            ...
            {report.attack_flow.map((step, i) => (
              <div key={step.label} className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-slate-500 text-xs uppercase tracking-wider font-medium mr-3 inline-block w-28 shrink-0">
                    {step.label}
                  </span>
                  <span className="font-mono text-sm text-slate-200 break-all">{step.value}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {(report.fixed_code || report.canonical_fix) && (
        <section className="mb-8">
          ...
          />
          {report.explanation && (
            <div className="mt-4 bg-blue-950/30 border border-blue-800/40 rounded-xl p-4 text-blue-200/80 text-sm leading-relaxed">
    ...
    <>
      {(report.why_blocked || report.explanation) && (
        <section className="mb-8">
          ...
          <div className="bg-green-950/30 border border-green-700/40 rounded-xl p-5 text-green-200/80 text-sm leading-relaxed">
            {report.why_blocked || report.explanation}
          </div>
          {report.fix_hint && (
            <div className="mt-2 bg-blue-950/30 border border-blue-800/40 rounded-xl p-4 text-blue-200/80 text-sm leading-relaxed">
      ...
      {report.submitted_code && (
        <section className="mb-8">
          ...
          <div className="rounded-lg border border-green-800/40 overflow-hidden">
            <CodeEditor value={report.submitted_code} readOnly language={lang} />
          </div>
        </section>
      )}
      ...
      {(report.fixed_code || report.canonical_fix) && (
        <section className="mb-8">
          ...
          <div className="rounded-lg border border-slate-700/60 overflow-hidden">
            <CodeEditor value={report.fixed_code || report.canonical_fix} readOnly language={lang} />
          </div>
        </section>
      )}
    </>
...
export default function Report() {
  ...
  const { report, loading, error } = useReportPolling(session_id)
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className={`rounded-xl border p-6 mb-8
        ${report.passed ? 'bg-green-900/20 border-green-700/50' : 'bg-red-900/20 border-red-700/50'}`}>
        <div className={`text-3xl font-bold mb-1 ${report.passed ? 'text-green-400' : 'text-red-400'}`}>
          {report.passed ? '✓ Vulnerability Fixed' : '✗ Still Vulnerable'}
        </div>
        <p className={`text-sm ${report.passed ? 'text-green-300/70' : 'text-red-300/70'}`}>
          {report.passed
      ...
      {report.passed ? <ReportPassed report={report} /> : <ReportFailed report={report} />}
      ...
      <div className="flex gap-3">
        ...
        >
          {report.passed ? 'Try Again' : 'Back to Challenge'}
        </Link>
        <Link
        ...
        >
          {report.passed ? 'Next Challenge →' : 'All Challenges'}
        </Link>
      </div>
    </div>
  )
